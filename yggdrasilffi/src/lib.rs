...
pub unsafe extern "C" fn inc_counter(
    ...
    let result = guard_result::<(), _>(|| {
        ...
        let name = get_str(name_ptr)?;
        let labels: Option<MetricLabels> = if labels_ptr.is_null() {
            None
        } else {
...
/// Increments a gauge metric by the given value (defaults to 1 if value is 0).
///
/// # Safety
///
/// The caller is responsible for ensuring all arguments are valid pointers.
...
/// `free_response` and passing in the pointer returned by this method. Failure to do so will result in a leak.
#[no_mangle]
pub unsafe extern "C" fn inc_gauge(
    engine_ptr: *mut c_void,
    name_ptr: *const c_char,
    value: i64,
    labels_ptr: *const c_char,
    ...
    let result = guard_result::<(), _>(|| {
        ...
        if let Some(labels) = labels {
            engine.inc_gauge_with_labels(&name, value, &labels);
        } else if value == 0 {
            engine.inc_gauge(&name);
        } else {
            engine.inc_gauge_by(&name, value);
        }
        Ok(Some(()))
    });
...
/// Decrements a gauge metric by the given value (defaults to 1 if value is 0).
///
/// # Safety
///
/// The caller is responsible for ensuring all arguments are valid pointers.
...
/// `free_response` and passing in the pointer returned by this method. Failure to do so will result in a leak.
#[no_mangle]
pub unsafe extern "C" fn dec_gauge(
    engine_ptr: *mut c_void,
    name_ptr: *const c_char,
    value: i64,
    labels_ptr: *const c_char,
    ...
    let result = guard_result::<(), _>(|| {
        ...
        if let Some(labels) = labels {
            engine.dec_gauge_with_labels(&name, value, &labels);
        } else if value == 0 {
            engine.dec_gauge(&name);
        } else {
            engine.dec_gauge_by(&name, value);
        }
        Ok(Some(()))
    });
