...
class UnleashEngine:
    def __init__(self):
        ...
        self.lib.inc_gauge.argtypes = [
            ctypes.c_void_p,
            ctypes.c_char_p,
            ctypes.c_int64,
            ctypes.c_char_p,
        ]
        self.lib.inc_gauge.restype = ctypes.POINTER(ctypes.c_char)

        self.lib.dec_gauge.argtypes = [
            ctypes.c_void_p,
            ctypes.c_char_p,
            ctypes.c_int64,
            ctypes.c_char_p,
        ]
        self.lib.dec_gauge.restype = ctypes.POINTER(ctypes.c_char)
    ...
    def inc_gauge(
        ...
        labels_json = json.dumps(labels).encode("utf-8") if labels else None
        actual_value = value if (labels or value != 1) else 0
        response_ptr = self.lib.inc_gauge(
            self.state,
            name.encode("utf-8"),
            actual_value,
            labels_json,
    ...
    def dec_gauge(
        ...
        labels_json = json.dumps(labels).encode("utf-8") if labels else None
        actual_value = value if (labels or value != 1) else 0
        response_ptr = self.lib.dec_gauge(
            self.state,
            name.encode("utf-8"),
            actual_value,
            labels_json,
    ...
    def define_histogram(
        ...
        buckets_json = json.dumps(buckets or []).encode("utf-8")
